...
void printNumEvents(TString catalog_code,Bool_t regenerate=true){

 	TChain *chain = new TChain("Delphes");
 	chainMaker(chain, catalog_code,regenerate);
 	ExRootTreeReader *treeReader = new ExRootTreeReader(chain);
 	...
 	cout << "--------------------------------------------" << endl;
 	cout << "Number of events in this sample set:" << numberOfEntries << endl;
 	cout << "********************************************" << endl;
}
...
TH1F * get_nMinusOneHist(vector <bool> SampleYield, vector <Double_t> minusOneVariableValuesVector, vector <Double_t> eventWeightsVector){
	...
	cout << "NUMBER_OF_ENTRIES gen_nMinusOneHist() thinks there is: " << numberOfEntries << endl;
	...
	for (int event = 0; event < numberOfEntries; event++) {
		cout << "hi i got here: " << endl;
		if (event > 98318) {

			cout << "event: " << event << " | minusOneVariableValuesVector[event] " << minusOneVariableValuesVector[event];
			cout << "eventWeightsVector[event] " << eventWeightsVector[event] << endl;

		}

		cout << "this is event: " << event << endl;
		cout << "SampleYield[event] is " << SampleYield[event] << endl;
		cout << "minusOneVariableValuesVector[event]" << minusOneVariableValuesVector[event] << endl;
		cout << "eventWeightsVector[event]" << eventWeightsVector[event] << endl;
...
vector<bool> getSampleYieldVector(TString cut_variable, map<TString, vector<Bool_t> > survivedCutMap){
	...
	// loop through the events and for all n - 1 variables in the map check to see if the event made it.
	// event must make it through all of the n - 1 cuts to remain with a "true" value in the eventsThatMadeIt map
...
void findSignificance(TString catalog_code, TString cut_variable, Bool_t regenerate=true){
	...
	for (Int_t jentry = 0; jentry < numberOfEntries; jentry++){
		...
		if (sample_type == "background"){
			...
			// this formula came from info Markus sent from background weighting paper
		...
		for (Int_t jet_index = 0; jet_index < jet_size; jet_index++){
			...
			if (b_tag == 1){
	            ...
	            // cut if it's low PT
    	        if (jet->P4().Pt()*factor(jet->PT,jet->Eta) < bjet_pt_cut) continue;
	...
	// assign directory name to put histograms depending on sample
	// all histograms will be saved to: /afs/cern.ch/user/h/habdalla/UserCode/habdallah/data

	TString histogram_location;
	if (catalog_code == "HHToGGBB") histogram_location = "/HHToGGBB/";
	else if(catalog_code == "ttB" ) histogram_location = "/ttB/";
	else if(catalog_code == "BB") histogram_location = "/BB/";
	...
	count_distrib_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"count_distribution.png");
	...
	invariantMassCanvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"BBGG_invariantMass.png");
	...
	muon_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"muon-stats.png");
	...
	electron_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"electron-stats.png");
	...
	photon_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"leading-photon-stats.png");
	...
	bjet_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"leading-bjet-stats.png");
	...
	bg_delta_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"bgDelR.png");
	...
	Higgs_canvas->SaveAs("/afs/cern.ch/user/h/habdalla/UserCode/habdallah/data"+histogram_location+"Higgs-stats.png");
	
	cout << endl << endl <<  "hi ther 0" << endl << endl;
	// -------------------
	...
	cout << endl << endl <<  "hi ther 1" << endl << endl;
	// loop through events
	// for (int ev = 1; ev < numberOfEntries; ev++){

	// 	cout << "this is event: " << ev << endl;
	// 	//cout << "SampleYield[event] is " << SampleYield[ev] << endl;
	// 	cout << "cutVariableValueMap[cut_variable][ev]" << cutVariableValueMap[cut_variable][ev] << endl;
	// 	cout << "eventWeights[ev]" << eventWeights[ev] << endl;
	...
	//if (SampleYield[event]) hi

	//nMinusOneHist->Draw();

	vector<Bool_t> didTheEventPassWithNoMinus; 

	didTheEventPassWithNoMinus = getSampleYieldVector("none", survivedCutMap); // get sample yield where there is no minus 1 cut variable
	
	cout << endl << endl <<  "hi ther 2" << endl << endl;
 ...
 //    /* std::cout << someVector[i]; ... */
	...
	// 	if (didTheEventPassWithNoMinus[event] == 1) {
	// 		cout << "hi im stopping now, press enter to continue" << endl;
	// 		cin.get(); 
	// 		numThatMadeAllCuts++;
	// 	}
	...
	// }
	cout << endl << endl <<  "hi ther 2" << endl << endl;
	//cout << "Number of events that made all cuts: " << numThatMadeAllCuts << endl;
	// ********************************************************************************
	// ********************************************************************************
	// ********************************************************************************
	// ********************************************************************************
	...
	// cout << endl << endl <<  "hi ther 2.5" << endl << endl;
	Double_t cross_section = .087; // CHANGE THIS IT IS WRONG!!!!
	// cout << endl << endl <<  "hi ther 2.6" << endl << endl;
	Int_t luminosity = 3000;

	cout << endl << endl <<  "hi ther 3" << endl << endl;
	...
	//e_times_a = (double)numThatMadeAllCuts/numberOfEntries;
	//cout << '\t' << "Luminosity: " << '\t' << '\t' << '\t' << "300 fb - 1" << endl;			
	...
	//cout << '\t' << '\t' << '\t'  << '\t' << e_times_a * 3000 * .034 << endl << endl;
	// S/B = e_times_a * lum * cross_ection
	cout << "-------------------------------------------------" << endl;
...
void tagAndProbeBJet(TString catalog_code,Bool_t regenerate=true){
...
void validateThisOneBackground(TString catalog_code, Double_t xsect, Double_t sample_set_info[]){
	...
	// fill the chain based on the catalog_code provided by the user
	chainMaker(chain, cat_code,true);
...
/*
	...
	for (Int_t jentry = 0; jentry < numberOfEntries; jentry++){
		...
		// cout << "event no: " << jentry << " event_weight: " << event->Weight << endl;
	...
	sample_set_info[0] = numEvents;
	sample_set_info[1] = bb_unnormalized_yield;
	sample_set_info[2] = gg_unnormalized_yield;
	sample_set_info[3] = bbgg_unnormalized_yield;
	sample_set_info[4] = bb_normalized_yield;
	sample_set_info[5] = gg_normalized_yield;
	sample_set_info[6] = bbgg_normalized_yield;
*/
	// cout << "WITHING validateThisOneBackground: " << endl << "---------------------------------" << endl;
 // 	cout << "num events: " << numEvents << endl;
	// cout << "bb-un-norm: " << bb_unnormalized_yield << endl;
	// cout << "gg-un-norm: " << gg_unnormalized_yield << endl;
	// cout << "bbgg-un-norm: " << bbgg_unnormalized_yield << endl;
	// cout << "bb-norm: " << bb_normalized_yield << endl;
	// cout << "gg-norm: " << gg_normalized_yield << endl;
	// cout << "bbgg-norm: " << bbgg_normalized_yield << endl;
	...
	// cout << "bb_un-normalized_yield: " << bb_unnormalized_yield << endl << endl;
	// cout << "gg_un-normalized_yield: " << gg_unnormalized_yield << endl << endl;
	// cout << "bbgg_un-normalized_yield: " << bbgg_unnormalized_yield << endl << endl;
	
	// cout << "----------------------------------------" << endl; 

	// cout << "bb_normalized_yield: " << bb_normalized_yield << endl << endl;
	// cout << "gg_normalized_yield: " << gg_normalized_yield << endl << endl;
	// cout << "bbgg_normalized_yield: " << bbgg_normalized_yield << endl << endl;
	...
	// TCanvas *ttBcanvas = new TCanvas("B","Good Rec B Jet Count Distribution",900,750);
...
void printTableHeading(){
	...
	Int_t lumi = 3000;
	cout << "------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
	cout << setw(4) << "BG" << " | " << setw(6) << "CONFIG" << " | "  << setw(22) << "SAMPLE SET" << " | " << setw(9) << " # Events" << " | " << setw(12) << " X-SECT";
	cout << " | " << setw(5) << "LUMI" << " | " << setw(12) << "BB_RAW" << " | " << setw(7) << "GG_RAW" << " | " << setw(9) << "BBGG_RAW";
	cout << " | " <<  setw(7) << "BB_NORM" << " | " << setw(7) << "GG_NORM" << " | " << setw(7) << "BBGG_NORM" << " | " << endl;

}

void printTableRow(TString BG, TString config, TString sample_set, Double_t xsect, Double_t lumi, Double_t sample_set_info[]){
...
void printAllSampleEventAmount(){
	...
	printNumEvents("BB-4p-0-300-v1510.txt",false);
	printNumEvents("BB-4p-1300-2100-v1510.txt",false);
	printNumEvents("BB-4p-2100-100000.txt",false);
	printNumEvents("BB-4p-300-700-v1510.txt",false);
	printNumEvents("BB-4p-700-1300-v1510.tx",false);
	printNumEvents("BBB-4p-0-600-v1510.txt",false);
	printNumEvents("BBB-4p-1300-100000-v1510.txt",false);
	printNumEvents("BBB-4p-600-1300-v1510.txt",false);
	printNumEvents("ttB-4p-0-900-v1510.txt",false);
	printNumEvents("ttB-4p-1600-2500-v1510.txt",false);
	printNumEvents("ttB-4p-2500-100000-v1510.txt",false);
	printNumEvents("ttB-4p-900-1600-v1510.txt",false);
...
// Double_t getWeight(Double_t xsect, Int_t lumi = 3000){
// 	Int_t weight = 1;
// 	return weight; 
// }