...
void validateThisOneSample(TString catalog_code, Double_t xsect, Double_t sample_set_info[]){
	...
	// fill the chain based on the catalog_code provided by the user
	chainMaker(chain, cat_code,false);
	...
	for (Int_t jentry = 0; jentry < numberOfEntries; jentry++){
		...
		// only weight if it's a background (i.e. not a signal - and there's only one signal sample catalog_code)
		if (catalog_code != "HHToGGBB_14TeV"){
